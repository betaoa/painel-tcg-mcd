...
async function clickText(page, pattern) {
  const target = page.getByText(pattern, { exact: false }).filter({ visible: true }).first(); await target.waitFor({ timeout: 30000 }); await target.click({ force: true });
}
async function selectSlicer(page, title, value) {
  const visual = page.locator('[class*="visual-container"]').filter({ hasText: new RegExp(title, "i") }).filter({ visible: true }).first();
  await visual.click({ force: true });
  const escaped = String(value).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const option = page.getByText(new RegExp(`^${escaped}import fs from "node:fs";
import path from "node:path";
import crypto from "node:crypto";
import XLSX from "xlsx";
import { chromium } from "playwright";
...
async function selectSlicer(page, title, value) {
  ...
  await visual.click({ force: true });
  const escaped = String(value).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
, "i")).filter({ visible: true }).last(); await option.waitFor({ timeout: 15000 }); await option.click();
